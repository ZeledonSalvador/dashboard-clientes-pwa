(function (DC) {
  ...
  function updateKPIFlujoDiaFrom(pesosJson) {
    ...
    const fmtTon = (kg) => {
      ...
      return `${ton.toLocaleString("es-SV", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Ton`;
    };
  ...
  function renderCharts(labels, baseSeries, azPack, promPack, resumenJson, productFilter, silent = false) {
    ...
    if (chFinalizados) {
      ...
      if (!realCount) {
        ...
        return; // ✅ clave: NO ejecutar autoZoom / widen
      }
    ...
    if (chRecibidos) {
      ...
      if (!realCount) {
        ...
        return; // ✅ CLAVE: no correr autoZoom / widen
      }
